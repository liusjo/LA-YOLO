...
class YOLOLoss(nn.Module):
    ...
    def Gcenter_loss(self, b1, b2,eps=0.01):
        """
        输入为：
        ----------
        b1: tensor, shape=(batch, feat_w, feat_h, anchor_num, 4), xywh
        ...
        返回为：
        -------
        Gcenter_loss=MSE(中心点/真实框长宽)
        """
        #----------------------------------------------------#
        #   求出预测框左上角右下角
        #----------------------------------------------------#
        x_loss=self.MSELoss(b1[..., 0],b2[..., 0])/(b2[..., 2]+eps)
        y_loss=self.MSELoss(b1[..., 1],b2[..., 1])/(b2[..., 3]+eps)
        #xy_loss=self.MSELoss(b1[..., :2]/(b2[..., 2:4]+eps),b2[..., :2]/(b2[..., 2:4]+eps))
    ...
    def forward(self, l, input, targets=None, y_true=None):
        ...
        if n != 0:
            ...
            loss_Gcenter=self.Gcenter_loss(pred_boxes, y_true[..., :4]).type_as(x)
            loss_Gcenter=torch.mean(loss_Gcenter[y_true[..., 4] == 1])
            ...
            loss        += loss_Gcenter*self.box_ratio*0.5+loss_loc * self.box_ratio + loss_cls * self.cls_ratio
            #-----------------------------------------------------------#
            #   计算置信度的loss
