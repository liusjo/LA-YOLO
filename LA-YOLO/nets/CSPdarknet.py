...
from nets.DCNConv import DeformConv2d
from nets.BiFormer import BiFormerBlock
...
class CSPDarknet(nn.Module):
    def __init__(self, base_channels, base_depth, phi, pretrained):
        ...
        #-----------------------------------------------#
        # print("使用DeformConv2d")
        self.dark2 = nn.Sequential(
            # 320, 320, 64 -> 160, 160, 128
            # 卷积可以改成DCNConv
            # DeformConv2d(inc = base_channels, outc = base_channels * 2, kernel_size=3, padding=1, stride=2),
        ...
        # print("使用swinT")
        # swinT
        self.swinT = SwinT(embed_dim=base_channels * 8) # #(bs, 640, 24, 24)

        # print("使用BiFormerBlock")
        # biformer
        # self.biformer = BiFormerBlock(dim=base_channels * 8)
        #-----------------------------------------------#
    ...
    def forward(self, x):
        ...
        x = self.dark2(x)
        feat0 = x
        # print('dark2.shape', x.shape)
        #-----------------------------------------------#
        #   dark3的输出为80, 80, 256，是一个有效特征层
        #-----------------------------------------------#
        x = self.dark3(x)
        # print('dark3.shape', x.shape)
        feat1 = x
        #-----------------------------------------------#
        #   dark4的输出为40, 40, 512，是一个有效特征层
        #-----------------------------------------------#
        x = self.dark4(x)
        # print('dark4.shape', x.shape)
        # print('dark4.type', type(x))
